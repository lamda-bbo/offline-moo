...
sys.path.extend([ROOT_DIR, BENCHMARK_DIR, CONFIG_DIR, SRC_DIR, UTILS_DIR])
...
def parse_args(args):
    ...
    setattr(
        args, "benchmark_path", os.path.join(BENCHMARK_DIR, benchmark_base, design_name)
    )
    with open(
        os.path.join(CONFIG_DIR, "benchmarks", f"{benchmark_base}.yaml"), "r"
    ) as f:
    ...
    algorithm_config_path = os.path.join(
        CONFIG_DIR, "algorithm", f"{args.algorithm}.yaml"
    )
    if os.path.exists(algorithm_config_path):
        with open(
            os.path.join(CONFIG_DIR, "algorithm", f"{args.algorithm}.yaml"), "r"
        ) as f:
    ...
    unique_token = "seed_{}_{}".format(
        args.seed, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    )

...
def plot(args):
    ...
    nrows, ncols = 1, 2
    fig, ax = plt.subplots(
        nrows, ncols, squeeze=False, sharey=True, figsize=(11 * ncols, 7 * nrows)
    )
    title = args.title
    ...
    for env_idx, mon in enumerate(monitors):
        ...
        ax[0, 0].plot(
            ...
            sm_10,
            label=envlabel
            + "; avg {:.1f} last {:.1f}".format(np.mean(sm_10), sm_10[-1]),
        )
        ...
        ax[0, 1].plot(
            ...
            sm_100,
            label=envlabel
            + "; avg {:.1f} last {:.1f}".format(np.mean(sm_100), sm_100[-1]),
        )
    ...
    ax[0, 0].set_xlabel(
        "Train Steps (total {})".format(total_train_steps), fontsize=xsize
    )
    ax[0, 1].set_title(title + ", Smoothed (w=100)", fontsize=titlesize)
    ax[0, 1].set_xlabel(
        "Train Time (in Hours {:.2f})".format(train_hours), fontsize=xsize
    )
    plt.tight_layout()

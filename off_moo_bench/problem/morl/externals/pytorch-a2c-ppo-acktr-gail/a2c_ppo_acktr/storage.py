...
class RolloutStorage(object):
    ...
    def update_utopian_z(self):
        ...
        self.utopian_z = torch.max(
            self.utopian_z, self.value_preds[:-1].max(axis=0).values.max(axis=0).values
        )
        self.nadir_z = self.returns[:-1].min(axis=0).values.min(axis=0).values
        self.nadir_z = torch.min(
            self.nadir_z, self.value_preds[:-1].min(axis=0).values.min(axis=0).values
        )

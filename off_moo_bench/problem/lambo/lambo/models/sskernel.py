...
class SSKernel(Kernel):
    ...
    def __init__(
        ...
        self.register_parameter(
            name="raw_match_decay", parameter=torch.nn.Parameter(torch.ones(1))
        )
        self.register_parameter(
            name="raw_gap_decay", parameter=torch.nn.Parameter(torch.ones(1))
        )
    ...
    def forward(self, x1, x2=None, diag=False, should_normalize=True, **kwargs):
        ...
        """
        if diag:
            return self.lengthscale * torch.ones(
                x1.shape[-2], dtype=x1.dtype, device=x1.device
            )
        ...
        if self.training:
            if self.cached_train_matches is not None:
                ...
                matches = (
                    x1.unsqueeze(-4).unsqueeze(-3) * x2.unsqueeze(-3).unsqueeze(-2)
                ).sum(-1)
        else:
            matches = (
                x1.unsqueeze(-4).unsqueeze(-3) * x2.unsqueeze(-3).unsqueeze(-2)
        ...
        d_mat = (
            self.precalc(lengthscale=self.gap_decay, maxlen=max_len)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        if should_normalize:
            if is_symmetric:
                kmat_diag = torch.diag_embed(
                    torch.diagonal(kmat, dim1=-2, dim2=-1).clamp(min=1e-10).pow(-0.5)
                )
                ...
                # if self.num_splits == 1:
                left_norm_term = (
                    torch.stack([self.forward(x, should_normalize=False) for x in x1])
                    .squeeze(-1)
                    .squeeze(-1)
                    .clamp(min=1e-4)
                )
                right_norm_term = (
                    torch.stack([self.forward(x, should_normalize=False) for x in x2])
                    .squeeze(-1)
                    .squeeze(-1)
                    .clamp(min=1e-4)
                )
    ...
    def __call__(self, x1, x2=None, diag=False, last_dim_is_batch=False, **params):
        return self.forward(
            x1=x1, x2=x2, diag=diag, last_dim_is_batch=last_dim_is_batch, **params
        )

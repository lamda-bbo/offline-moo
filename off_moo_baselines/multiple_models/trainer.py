    ...
    def _evaluate_performance(
        ...
        with torch.no_grad():
            ...
            if len(test_loader) != 0:
                ...
                for i in range(self.n_obj):
                    statistics[
                        f"model_{self.which_obj}/test/rank_corr_{i + 1}"
                    ] = test_corr[i].item()
...
class ConservativeObjectiveTrainer(SingleModelBaseTrainer):
    ...
    def train_step(self, x, y, statistics):
        ...
        statistics[
            f"model_{self.which_obj}/train/overestimation/mean"
        ] = overestimation.mean()
        statistics[
            f"model_{self.which_obj}/train/overestimation/std"
        ] = overestimation.std()
        statistics[
            f"model_{self.which_obj}/train/overestimation/max"
        ] = overestimation.max()
    ...
    def optimize(self, x, steps, **kwargs):
        ...
        for _ in range(steps):
            ...
            with torch.no_grad():  # rep_model is used in inference mode, no need to track gradients
                ...
                xt_rep = xt_rep / (
                    torch.sqrt(torch.sum(xt_rep**2, dim=-1, keepdim=True) + 1e-6)
                    + 1e-6
                )
...
class RoMATrainer(SingleModelBaseTrainer):
    ...
    def update_step(self, statistics):
        ...
        statistics[
            f"model_{self.which_obj}/update/travelled"
        ] = travelled.item()  # Convert to Python scalar with .item()
...
class TriMentoringTrainer(SingleModelBaseTrainer):
    ...
    def train_single_model(
        ...
        from off_moo_baselines.multiple_models.external.tri_mentoring_utils import (
        ...
        for epoch in range(self.n_epochs):
            ...
            statistics[
                f"model_{self.which_obj}_{model.seed}/train/loss/mean"
            ] = np.array(losses).mean()
            statistics[
                f"model_{self.which_obj}_{model.seed}/train/loss/std"
            ] = np.array(losses).std()
            statistics[
                f"model_{self.which_obj}_{model.seed}/train/loss/max"
            ] = np.array(losses).max()
            ...
            with torch.no_grad():
                ...
                statistics[
                    f"model_{self.which_obj}_{model.seed}/train/mse"
                ] = train_mse.item()
                for i in range(self.n_obj):
                    statistics[
            ...
            with torch.no_grad():
                ...
                statistics[
                    f"model_{self.which_obj}_{model.seed}/valid/mse"
                ] = val_mse.item()
                for i in range(self.n_obj):
                    statistics[
                ...
                if len(test_loader) != 0:
                    ...
                    statistics[
                        f"model_{self.which_obj}_{model.seed}/test/mse"
                    ] = test_mse.item()
                    for i in range(self.n_obj):
                        statistics[

...
def adjust_bpr(proxy, pairs, labels, x0, y0, soft_label=True):
    ...
    if soft_label:
        with higher.innerloop_ctx(proxy, opt) as (fproxy, diffopt):
            ...
            ce_loss = -torch.mean(
                labels * torch.log(prob + 1e-9)
                + (1 - labels) * torch.log(1 - prob + 1e-9)
            )
    ...
    ce_loss = -torch.mean(
        labels * torch.log(prob + 1e-9) + (1 - labels) * torch.log(1 - prob + 1e-9)
    )
    opt.zero_grad()
    ce_loss.backward()
    opt.step()
...
def adjust_proxy(
    ...
    candidate_neighbors = candidate_neighbors + 0.1 * torch.randn(
        candidate_neighbors.shape
    ).to(candidate.device)

...
from off_moo_baselines.mobo.surrogate_problem import LCB_Perm_Problem, LCB_Seq_Problem
...
class MOBOPermutation:
    ...
    def run(self) -> ndarray:
        model = self._get_model(self.X_init, self.Y_init)
        problem = LCB_Perm_Problem(self.dim, self.num_obj, model)
        print('----nsag2,solving...-----')
...
class MOBOSequence:
    ...
    def run(self) -> ndarray:
        ...
        problem = LCB_Seq_Problem(self.dim, self.num_obj, model, self.xl, self.xu)
        print('----nsag2,solving...-----')
        tmp_config = deepcopy(self.config)
        tmp_config.update({

...
from off_moo_bench.evaluation.metrics import get_OI, get_SI, get_SO, get_SOW, hv, igd
...
class IGDRecordCallback:
    ...
    def __call__(self, algorithm):
        ...
        # if y_real.shape[1] <= 3:
        if y_real.shape[1] <= 10:
            nadir_point = self.task.nadir_point

            if self.config["normalize_ys"]:
                pareto_front = self.task.normalize_y(
                    self.task.problem.get_pareto_front(), normalization_method="min-max"
                )
                nadir_point = self.task.normalize_y(
                    nadir_point, normalization_method="min-max"
                )
            ...
            self.save_data.append(igd_value)

            self.SI.append(
                get_SI(
                    [-1 * element for element in self.save_data],
                    self.n_gen,
                    self.config["solver_n_gen"],
                    None,
                )
            )
            self.OI.append(
                ...
                wandb.log(
                    {
                        ...
                        "Iteration_nums": self.config["solver_n_gen"],
                        "seed": self.config["seed"],
                        "model": self.config["model"],
                    }
                )
                for critical_index in [24, 49, 74, 99, 124, 149, 174, 199]:
                    wandb.log(
                        {
                            f"SI_{critical_index+1}": self.SI[critical_index],
                            f"OI_{critical_index+1}": self.OI[critical_index],
                            f"SO_{critical_index+1}": self.SO[critical_index],
                            f"SOW_{critical_index+1}": self.SOW[critical_index],
                        }
                    )
                    wandb.log(
                        {
                            f"hv_{critical_index+1}": self.hv_data[critical_index],
                            f"igd_{critical_index+1}": self.save_data[critical_index],
                        }
                    )

                wandb.log({"search_step": self.n_gen})

            metrics_data = {}

            for critical_index in [24, 49, 74, 99, 124, 149, 174, 199]:
                metrics_data[f"iteration_{critical_index+1}"] = {
                    "SI": self.SI[critical_index],
                    "OI": self.OI[critical_index],
                    "SO": self.SO[critical_index],
                    "SOW": self.SOW[critical_index],
                    "hv": self.hv_data[critical_index],
                    "igd": self.save_data[critical_index],
                }

            with open(
                os.path.join(self.save_dir, "..", "stability_results.json"), "w"
            ) as f:
